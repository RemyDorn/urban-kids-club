  ...
  await Promise.all(activeBlocks.map(async block=>{
    try{
      ...
      ;(sr.data||[]).forEach(sess=>{
        ...
        sessionDates[d].push({title:act.title,start:sess.startTime||slot.startTime||block.recurringTime,end:sess.endTime||slot.endTime||'',cat:act.category,age:(act.ageRange?.min!=null?act.ageRange.min:0)+'-'+(act.ageRange?.max!=null?act.ageRange.max:0)+' '+(act.ageRange?.unit==='months'?'M.':'J.'),price:act.pricing?.[0]?.amount?act.pricing[0].amount.toFixed(0)+'\\u20AC':'',color:act.color||BC,desc:act.description||'',hasActiveBlock:true,blockId:block.id,activityId:act.id})
      })
    }catch(e){}
  }))
    ...
    for(let cur=new Date(start);cur<=end;cur.setDate(cur.getDate()+1)){
      ...
      a.schedule.slots.forEach(s=>{
        if(DN[s.day]===cur.getDay()){
          ...
          sessionDates[ds].push({title:a.title,start:s.startTime,end:s.endTime,cat:a.category,age:(a.ageRange?.min!=null?a.ageRange.min:0)+'-'+(a.ageRange?.max!=null?a.ageRange.max:0)+' '+(a.ageRange?.unit==='months'?'M.':'J.'),price:a.pricing?.[0]?.amount?a.pricing[0].amount.toFixed(0)+'\\u20AC':'',color:a.color||BC,desc:a.description||'',hasActiveBlock:false})
        }
      })
    }
  })
  ...
  function renderStep(){
    ...
    if(step===1){
      m.querySelector('#btnNext1').onclick=function(){
        ...
        // Age-Validation gegen Activity ageRange
        var ar=course.ageRange||{}
        if(ar.min!=null||ar.max!=null){
          var unit=ar.unit||'years'
          var byr=parseInt(ckY)
          var nowY=new Date().getFullYear()
          var ageY=nowY-byr
          var minY=unit==='months'?(ar.min||0)/12:(ar.min||0)
          var maxY=unit==='months'?(ar.max||0)/12:(ar.max||99)
          var label=unit==='months'?(ar.min+'–'+ar.max+' Monate'):(ar.min+'–'+ar.max+' Jahre')
          if(ageY>maxY+1){showMsg(ckF+' (geb. '+byr+') ist zu alt für diesen Kurs ('+label+').','error');return}
          if(ageY<minY-1){showMsg(ckF+' (geb. '+byr+') ist zu jung für diesen Kurs ('+label+').','error');return}
        }
...
(async()=>{
  ...
  try{
    ...
    var trimS=function(t){return t?t.split(':').slice(0,2).join(':'):''};var schedule=function(a){if(a.schedule?.type!=='recurring'||!a.schedule.slots)return '';var days={MO:'Montag',TU:'Dienstag',WE:'Mittwoch',TH:'Donnerstag',FR:'Freitag',SA:'Samstag',SU:'Sonntag'};return a.schedule.slots.map(function(s){return (days[s.day]||s.day)+' '+trimS(s.startTime)+'-'+trimS(s.endTime)+' Uhr'}).join(', ')}
    var ageStrFn=function(a){var u=a.ageRange?.unit==='months'?'Monate':'Jahre';return (a.ageRange?.min!=null?a.ageRange.min:'?')+'-'+(a.ageRange?.max!=null?a.ageRange.max:'?')+' '+u}
    app.innerHTML=published.map(a=>'<div class="course"><div class="course-info"><div class="course-title">'+esc(a.title)+'</div><div class="course-meta">'+ageStrFn(a)+(schedule(a)?' · '+schedule(a):'')+(a.pricing?.[0]?.amount?' · '+a.pricing[0].amount+'\\u20AC':'')+'<span id="rb-'+a.id+'" style="margin-left:6px"></span></div>'+(a.description?'<p style="font-size:13px;color:#3C2225;margin-top:8px;margin-bottom:0">'+esc(a.description.substring(0,120))+(a.description.length>120?'...':'')+'</p>':'')+'</div><a class="book-btn" href="/embed/'+slug+'/calendar?activity='+a.id+'">Buchen</a></div>').join('')+'<div style="text-align:center;padding:8px 0;font-size:10px"><a href="https://urbankidsclub.de" target="_blank" rel="noopener" style="color:#94a3b8;text-decoration:none">Powered by Urban Kids Club</a></div>'
...
(async function(){
  ...
  try{
    ...
    const{data}=await r.json()
    // Alle nicht-archivierten/stornierten Kurse zeigen. Buchbar nur active/published. Sonst → Warteliste.
    const allowed=['published','active','paused','draft','planning','in_planning']
    const a=data.find(function(x){return x.id===actId&&allowed.indexOf(x.status)>=0})
    if(!a){app.innerHTML='<div class="empty">Kurs nicht verfügbar.</div>';return}
    const isBookable=a.status==='published'||a.status==='active'
    const isPaused=a.status==='paused'||a.pausedAt
    const isUnpublished=a.status==='draft'||a.status==='planning'||a.status==='in_planning'
    const showWaitlist=!isBookable
    const schedule=a.schedule?.type==='recurring'?a.schedule.slots?.map(function(s){
      var days={MO:'Montag',TU:'Dienstag',WE:'Mittwoch',TH:'Donnerstag',FR:'Freitag',SA:'Samstag',SU:'Sonntag'}
      var trim=function(t){return t?t.split(':').slice(0,2).join(':'):''}
      return (days[s.day]||s.day)+' '+trim(s.startTime)+'-'+trim(s.endTime)+' Uhr'
    }).join(', '):''
    const ageUnit=a.ageRange?.unit==='months'?'Monate':'Jahre'
    const ageStr=(a.ageRange?.min!=null?a.ageRange.min:'?')+'-'+(a.ageRange?.max!=null?a.ageRange.max:'?')+' '+ageUnit
    ...
    var statusNote=''
    if(isPaused){
      statusNote='<div style="background:#FBE5D6;color:#8B4A2A;padding:10px 14px;border-radius:10px;font-size:13px;margin-bottom:14px;">⏸ Kurs aktuell pausiert &mdash; trag dich auf die Warteliste ein, wir benachrichtigen dich beim Wieder-Start.</div>'
    } else if(isUnpublished){
      statusNote='<div style="background:#F4E9D1;color:#8B4A2A;padding:10px 14px;border-radius:10px;font-size:13px;margin-bottom:14px;">📝 Kurs in Vorbereitung &mdash; lass dich vormerken, wir informieren dich beim Start.</div>'
    }
    var ctaLabel=isBookable?'Jetzt buchen':'Auf die Warteliste'
    var ctaHref='/embed/'+slug+'/calendar?activity='+esc(a.id)+(showWaitlist?'&waitlist=1':'')
    app.innerHTML='<div class="card"><div class="card-header"><div class="title">'+esc(a.title)+'</div><div class="meta"><span>'+ageStr+'</span>'+(schedule?'<span>'+esc(schedule)+'</span>':'')+'</div><div id="rating"></div></div><div class="card-body">'+statusNote+(a.description?'<div class="desc">'+esc(a.description)+'</div>':'')+(price&&isBookable?'<div class="price">'+price+pkg+'</div>':'')+'<a class="btn" href="'+ctaHref+'">'+ctaLabel+'</a></div></div>'
...
<div class="card" id="card">
  ...
  <!-- Step 2: Select child (hidden initially) -->
  <div id="step2" style="display:none">
    <div class="logo">👋</div>
    <h1>Wer ist dabei?</h1>
    <p class="subtitle">Wähle dein Kind aus</p>
    <div id="step2List" class="child-list"></div>
    <button type="button" onclick="goStep1()" class="btn" style="background:transparent;color:#8B7355;margin-top:6px">← Zurück</button>
  </div>

  <!-- Step 3: Select course for that child -->
  <div id="step3" style="display:none">
    <div class="logo">📚</div>
    <h1 id="step3Title">Welcher Kurs?</h1>
    <p class="subtitle" id="step3Subtitle">Wähle den Kurs für heute</p>
    <div id="step3List" class="child-list"></div>
    ...
    <button type="button" onclick="goStep2()" class="btn" style="background:transparent;color:#8B7355;margin-top:6px">← Anderes Kind wählen</button>
  </div>
...
let _email='',_bookings=[],_selectedChild=null;
...
function distinctChildren(){
  ...
  for(const b of _bookings){
    const k=(b.childName||'').trim()||'Kind';
    if(!map[k])map[k]={name:k,total:0,done:0,unpaid:0};
    map[k].total++;
    if(b.alreadyCheckedIn)map[k].done++;
    if(!b.alreadyCheckedIn&&b.paymentStatus!=='paid')map[k].unpaid++;
  }
...
function goStep2(){
  ...
  const kids=distinctChildren();
  // Auto-skip if only one child
  if(kids.length===1){ pickChild(kids[0].name); return; }
  const el=document.getElementById('step2List');
  let html='';
  for(const k of kids){
    const allDone=k.done===k.total;
    const cls='child-item'+(allDone?' already':'');
    const meta=allDone?'Heute komplett eingecheckt ✓'
      :(k.total-k.done)+' Termin'+((k.total-k.done)===1?'':'e')+' offen'+(k.unpaid>0?' · '+k.unpaid+' unbezahlt':'');
    const metaCls=allDone?'paid':(k.unpaid>0?'unpaid':'paid');
    html+='<div class="'+cls+'" '+(allDone?'':'onclick="pickChild(\\''+esc(k.name).replace(/'/g,"\\\\'")+'\\')"')+'>';
    html+='<div class="child-cb">'+(allDone?'✓':'›')+'</div>';
    html+='<div class="child-info">';
    html+='<div class="child-name">'+esc(k.name)+'</div>';
    html+='<div class="child-payment '+metaCls+'">'+esc(meta)+'</div>';
    html+='</div></div>';
  }
  el.innerHTML=html;
}
...
function pickChild(name){
  _selectedChild=name;
  // mark child's bookings, clear others
  _bookings.forEach(function(b){ b._selected=false; });
  document.getElementById('step2').style.display='none';
  document.getElementById('step3').style.display='';
  document.getElementById('step3Title').innerHTML='Kurs wählen';
  document.getElementById('step3Subtitle').textContent='Termine heute für '+name;
  // Auto-select if only one course bookable
  const own=_bookings.filter(function(b){return (b.childName||'')===name && !b.alreadyCheckedIn;});
  if(own.length===1){
    const i=_bookings.indexOf(own[0]);
    _bookings[i]._selected=true;
  }
  renderCourses();
}

function renderCourses(){
  const el=document.getElementById('step3List');
  let html='';
  let any=false;
  for(let i=0;i<_bookings.length;i++){
    const b=_bookings[i];
    if((b.childName||'')!==_selectedChild)continue;
    any=true;
    ...
    html+='<div class="child-info">';
    html+='<div class="child-name">'+esc(b.activityTitle)+'</div>';
    html+='<div class="child-course">'+esc(b.childName)+'</div>';
    if(done){
  ...
  if(!any){
    html='<div style="padding:20px;color:#8B7355;font-size:14px;text-align:center">Keine Kurse heute für '+esc(_selectedChild||'')+'.</div>';
  }
  el.innerHTML=html;
  const anySelected=_bookings.some(function(b){return b._selected&&!b.alreadyCheckedIn&&(b.childName||'')===_selectedChild;});
...
// Team-Invite Accept Page (mobile-friendly, double-opt-in via Supabase magic-link)
function generateTeamAcceptHtml(token: string): string {
  const safeToken = token.replace(/[^a-fA-F0-9-]/g, '')
  const supabaseUrl = process.env.SUPABASE_URL || ''
  const supabaseKey = process.env.SUPABASE_ANON_KEY || ''
  return `<!DOCTYPE html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Team-Einladung annehmen</title>
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&family=Instrument+Serif:ital@0;1&display=swap" rel="stylesheet">
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:'Inter',sans-serif;min-height:100vh;background:linear-gradient(135deg,#faf9f8 0%,#f0ebe6 100%);display:flex;align-items:center;justify-content:center;padding:20px;color:#3C2225}
.card{background:#fff;border-radius:24px;box-shadow:0 8px 32px rgba(0,0,0,0.08);max-width:520px;width:100%;padding:36px 32px}
.kicker{font-size:11px;letter-spacing:.18em;text-transform:uppercase;color:#8B7355;margin-bottom:6px;text-align:center}
h1{font-family:'Instrument Serif',serif;font-size:28px;color:#3C2225;margin-bottom:8px;font-weight:400;text-align:center}
h1 em{color:#B5533A;font-style:italic}
.studio{text-align:center;color:#8B7355;font-size:14px;margin-bottom:24px}
.note{background:#FFF9F5;padding:14px 18px;border-left:3px solid #D4956A;border-radius:8px;font-size:13px;color:#3C2225;margin:16px 0;font-style:italic;line-height:1.5}
.row{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px}
.field{display:flex;flex-direction:column;gap:5px;margin-bottom:12px}
.field.full{grid-column:1/-1}
label{font-size:12px;font-weight:500;color:#8B7355}
input,select{padding:11px 14px;border:1.5px solid #e8e0d8;border-radius:10px;font-size:14px;font-family:inherit;outline:none;transition:border-color .2s;background:#fff}
input:focus,select:focus{border-color:#D4956A}
input:read-only{background:#f7f3ee;color:#8B7355}
.btn{width:100%;padding:14px;background:#D4956A;color:#fff;border:0;border-radius:12px;font-size:15px;font-weight:600;cursor:pointer;font-family:inherit;transition:background .2s;margin-top:8px}
.btn:hover{background:#c4854a}
.btn:disabled{background:#ccc;cursor:not-allowed}
.btn-ghost{background:transparent;color:#8B7355;margin-top:6px}
.error{color:#dc2626;font-size:13px;margin-top:12px;padding:10px;background:#fef2f2;border-radius:8px}
.success{color:#059669;font-size:13px;margin-top:12px;padding:10px;background:#ecfdf5;border-radius:8px}
.info{color:#3C2225;font-size:13px;margin-top:12px;padding:12px;background:#fff9f5;border-radius:8px;line-height:1.5}
...
@keyframes spin{to{transform:rotate(360deg)}}
.steps{display:flex;justify-content:center;gap:6px;margin:0 0 22px;}
.step{width:36px;height:4px;background:#e8e0d8;border-radius:2px}
.step.active{background:#D4956A}
.step.done{background:#059669}
.footer{text-align:center;color:#94a3b8;font-size:11px;margin-top:24px}
hr{border:none;border-top:1px solid #f2e6e2;margin:18px 0}
</style></head><body>
<div class="card">
  <!-- STATE: Loading -->
  <div id="loading" style="text-align:center;padding:40px 0">
    <div class="spinner" style="border-color:#D4956A;border-top-color:transparent;width:32px;height:32px"></div>
    <div style="margin-top:14px;color:#8B7355;font-size:13px">Einladung wird geprüft…</div>
  </div>

  <!-- STATE: Invalid (expired/used/wrong) -->
  <div id="invalid" style="display:none;text-align:center">
    <div style="font-size:48px;margin-bottom:12px">⚠️</div>
    <h1 id="invalidTitle">Einladung ungültig</h1>
    <p id="invalidMsg" style="color:#8B7355;margin-top:12px">Die Einladung konnte nicht geladen werden.</p>
    <p style="color:#8B7355;font-size:13px;margin-top:18px">Bitte den Anbieter um eine neue Einladung.</p>
  </div>

  <!-- STATE: Step 1 — Confirm email (send magic-link) -->
  <div id="step1" style="display:none">
    <div class="steps"><div class="step active"></div><div class="step"></div><div class="step"></div></div>
    <div class="kicker">Schritt 1 von 3</div>
    <h1>E-Mail <em>bestätigen</em></h1>
    <p class="studio">Du wurdest in <strong id="step1Studio">…</strong> eingeladen.</p>
    <div class="info">
      Aus Sicherheitsgründen senden wir einen Bestätigungs-Link an deine E-Mail-Adresse. Klicke darauf, um fortzufahren.
    </div>
    <div class="field" style="margin-top:18px">
      <label>Eingeladene E-Mail-Adresse</label>
      <input type="email" id="email" readonly>
    </div>
    <button class="btn" id="sendMagicBtn">Bestätigungs-Link senden</button>
    <div id="step1Error"></div>
  </div>

  <!-- STATE: Magic-Link sent — waiting -->
  <div id="step1sent" style="display:none">
    <div class="steps"><div class="step done"></div><div class="step active"></div><div class="step"></div></div>
    <div class="kicker">Schritt 2 von 3</div>
    <h1>Schau in dein <em>Postfach</em></h1>
    <p class="studio">Wir haben dir gerade einen Bestätigungs-Link an <strong id="emailDisplay">…</strong> gesendet.</p>
    <div class="info">
      <strong>Klicke den Link in der E-Mail</strong> — du kommst dann automatisch hierher zurück und kannst dein Profil ausfüllen.
    </div>
    <button class="btn btn-ghost" onclick="window.location.reload()">Link nicht erhalten? Neu laden</button>
  </div>

  <!-- STATE: Step 2 — Profile form (after magic-link click) -->
  <div id="step2" style="display:none">
    <div class="steps"><div class="step done"></div><div class="step done"></div><div class="step active"></div></div>
    <div class="kicker">Schritt 3 von 3</div>
    <h1>Dein <em>Profil</em></h1>
    <p class="studio">Fast geschafft! Bitte fülle deine Daten aus.</p>
    <form id="profileForm">
      <div class="row">
        <div class="field"><label>Vorname *</label><input type="text" id="firstName" required maxlength="100"></div>
        <div class="field"><label>Nachname *</label><input type="text" id="lastName" required maxlength="100"></div>
      </div>
      <div class="field"><label>Telefon</label><input type="tel" id="phone" maxlength="30" placeholder="+49 …"></div>
      <div class="row">
        <div class="field"><label>Geburtsdatum</label><input type="date" id="birthDate"></div>
        <div class="field"><label>Eintrittsdatum</label><input type="date" id="hiredAt"></div>
      </div>
      <hr>
      <div class="field"><label>Straße + Hausnummer</label><input type="text" id="addrStreet" maxlength="200"></div>
      <div class="row">
        <div class="field"><label>PLZ</label><input type="text" id="addrZip" maxlength="20"></div>
        <div class="field"><label>Stadt</label><input type="text" id="addrCity" maxlength="100"></div>
      </div>
      <hr>
      <div class="field"><label>Anstellungsart</label>
        <select id="employmentType">
          <option value="employee">Festangestellt</option>
          <option value="freelance">Freelancer / Honorarkraft</option>
          <option value="volunteer">Ehrenamt</option>
        </select>
      </div>
      <hr>
      <div class="kicker" style="text-align:left;margin-bottom:8px">Notfallkontakt</div>
      <div class="row">
        <div class="field"><label>Name</label><input type="text" id="ecName" maxlength="100"></div>
        <div class="field"><label>Telefon</label><input type="tel" id="ecPhone" maxlength="30"></div>
      </div>
      <div class="field"><label>Beziehung (z.B. Partner, Mutter)</label><input type="text" id="ecRelation" maxlength="50"></div>
      <button type="submit" class="btn" id="submitBtn">Profil speichern &amp; beitreten</button>
    </form>
  ...
  <div id="done" style="display:none;text-align:center">
    ...
    <h1>Willkommen im <em>Team</em>!</h1>
    <p style="color:#3C2225;margin-top:12px">Du bist jetzt Teil von <strong id="doneStudio">…</strong>.</p>
    <p style="color:#8B7355;margin-top:18px;font-size:13px">Du kannst dich ab sofort mit deiner E-Mail einloggen.</p>
    <a href="/" class="btn" style="display:inline-block;text-decoration:none;text-align:center;margin-top:18px;">Zum Login</a>
  </div>
...
function show(id) {
  ['loading','invalid','step1','step1sent','step2','done'].forEach(function(x){
...
async function init() {
  // Check if we already have a session (came back from magic-link)
  let session = null;
  if (sb) {
    // Parse hash params (Supabase puts tokens in URL hash)
    if (window.location.hash && window.location.hash.includes('access_token')) {
      ...
      if (accessToken && refreshToken) {
        try {
          const { data } = await sb.auth.setSession({ access_token: accessToken, refresh_token: refreshToken });
  ...
  // Verify invite
  try {
    const r = await fetch('/api/public/team-invites/' + TOKEN);
    if (!r.ok) {
      const err = await r.json().catch(function(){return {}});
      document.getElementById('invalidMsg').textContent = err.error || 'Einladung nicht gefunden.';
      show('invalid');
      return;
    }
    ...
    document.getElementById('invalidMsg').textContent = 'Verbindungsfehler. Bitte später erneut versuchen.';
    show('invalid');
    return;
  }

  // If session matches invited email → go to profile step
  if (session && session.user && session.user.email && session.user.email.toLowerCase() === inviteData.invitedEmail.toLowerCase()) {
    show('step2');
    return;
  }

  // Otherwise show step 1
  document.getElementById('step1Studio').textContent = inviteData.providerName;
  document.getElementById('email').value = inviteData.invitedEmail;
  document.getElementById('emailDisplay').textContent = inviteData.invitedEmail;
  show('step1');
}

document.getElementById('sendMagicBtn').addEventListener('click', async function() {
  const btn = this;
  btn.disabled = true; btn.innerHTML = '<span class="spinner"></span>Wird gesendet…';
  document.getElementById('step1Error').innerHTML = '';
  try {
    const r = await fetch('/api/public/team-invites/' + TOKEN + '/send-confirmation', {
    ...
    showError('step1Error', e.message || 'Senden fehlgeschlagen');
    btn.disabled = false; btn.textContent = 'Bestätigungs-Link senden';
  }
});
...
document.getElementById('profileForm').addEventListener('submit', async function(e) {
  ...
  if (!sb) { showError('step2Error', 'Auth nicht konfiguriert'); btn.disabled = false; btn.textContent = 'Profil speichern & beitreten'; return; }
  const { data: { session } } = await sb.auth.getSession();
  if (!session || !session.access_token) {
    showError('step2Error', 'Sitzung abgelaufen. Bitte Seite neu laden.');
    btn.disabled = false; btn.textContent = 'Profil speichern & beitreten';
    return;
  }
  ...
  const profile = {
    ...
    emergencyContact: {
      ...
      relation: document.getElementById('ecRelation').value.trim(),
    },
  };

  try {
    const r = await fetch('/api/public/team-invites/' + TOKEN + '/accept', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + session.access_token },
      body: JSON.stringify({ profile: profile }),
    });
    const j = await r.json();
    if (!r.ok) throw new Error(j.error || 'Annahme fehlgeschlagen');
    document.getElementById('doneStudio').textContent = inviteData.providerName;
    show('done');
  } catch(e) {
...
const server = createServer(async (req, res) => {
  ...
  // Team-Invite Accept Page: /team/accept/:token
  if (path.startsWith('/team/accept/')) {
    const token = path.split('/')[3] || ''
    if (token) {
      res.setHeader('Content-Type', 'text/html; charset=utf-8')
      res.setHeader('Cache-Control', 'no-cache')
      res.statusCode = 200
      res.end(generateTeamAcceptHtml(token))
      return
    }
  }
