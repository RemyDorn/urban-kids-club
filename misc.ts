...
export function registerMiscRoutes(router: Router) {
  ...
  // Activity-Feed für Glocke (Topbar) — letzte 30 Provider-Events (Buchungen, Cancellations, Mom-Graph, Messages)
  // (2026-05-01 nachgezogen — Endpoint fehlte, Glocke war 404)
  router.get('/api/activity-feed', async (req, res) => {
    const auth = await requireAuth(req, res)
    if (!auth) return
    const sb = getServiceClient()

    // 1. Provider-Notifications (new_booking, booking_cancelled etc.) aus notifications-Tabelle
    const { data: notifs } = await sb.from('notifications')
      .select('id, type, title, body, data, sent_at, read')
      .eq('recipient_type', 'provider').eq('recipient_id', auth.providerId)
      .order('sent_at', { ascending: false })
      .limit(40)

    // 2. Recent parent-sent messages (not yet covered by notifications)
    const { data: msgs } = await sb.from('messages')
      .select('id, body, sent_at, parent_id')
      .eq('provider_id', auth.providerId).eq('type', 'parent')
      .order('sent_at', { ascending: false })
      .limit(15)
    const parentIds = [...new Set((msgs ?? []).map((m: any) => m.parent_id).filter(Boolean))]
    const { data: parentRows } = parentIds.length
      ? await sb.from('parents').select('id, name').in('id', parentIds)
      : { data: [] }
    const parentMap = new Map((parentRows ?? []).map((p: any) => [p.id, p.name]))

    // Map notifications to feed events
    const events: any[] = []
    for (const n of (notifs ?? [])) {
      const t = n.type || 'booking'
      let kicker = 'Buchung'
      let mappedType = 'booking'
      if (t.includes('cancel')) { kicker = 'Stornierung'; mappedType = 'cancel' }
      else if (t.includes('signup') || t.includes('new_customer') || t.includes('parent')) { kicker = 'Neuer Kunde'; mappedType = 'momgraph' }
      else if (t.includes('waitlist')) { kicker = 'Warteliste'; mappedType = 'waitlist' }
      else if (t.includes('mom_graph') || t.includes('invite')) { kicker = 'Mom-Graph'; mappedType = 'momgraph' }
      else if (t.includes('message')) { kicker = 'Nachricht'; mappedType = 'message' }
      else if (t.includes('booking') || t === 'new_booking') { kicker = 'Buchung'; mappedType = 'booking' }
      events.push({
        id: n.id,
        type: mappedType,
        kicker,
        ...
        at: n.sent_at,
        read: !!n.read,
        data: n.data || null,  // Frontend kann via data.sessionId / data.parentId direkt navigieren
      })
    }
    for (const m of (msgs ?? [])) {
      const parentName = parentMap.get(m.parent_id) || 'Eltern'
      events.push({
        id: 'msg-' + m.id,
        type: 'message',
        kicker: 'Nachricht',
        title: parentName + ' schreibt',
    ...
    // Sort by 'at' DESC, return top 30
    events.sort((a, b) => (b.at || '').localeCompare(a.at || ''))
    res.json({ data: events.slice(0, 30) })
  })

  // Threads-Übersicht für Provider-Postfach: alle Konversationen gruppiert nach parent_id
  // (2026-04-30 nachgezogen — Endpoint fehlte komplett, Dashboard-Postfach blieb leer)
  router.get('/api/messages/threads', async (req, res) => {
    const auth = await requireAuth(req, res)
    if (!auth) return
    const sb = getServiceClient()
    const { data: msgs, error } = await sb.from('messages')
      .select('id, parent_id, type, body, subject, read, sent_at')
      .eq('provider_id', auth.providerId)
      .order('sent_at', { ascending: false, nullsFirst: false })
      .limit(2000)
    if (error) return res.error(500, error.message)
    if (!msgs || msgs.length === 0) return res.json({ data: [] })

    // Group by parent_id; include messages array (Frontend pfOpen erwartet thread.messages)
    const byParent = new Map<string, any>()
    for (const m of msgs) {
      if (!m.parent_id) continue
      // Normalize 'direct' → 'provider' so Frontend-Filter konsistent funktionieren
      const normalizedType = m.type === 'direct' ? 'provider' : (m.type || 'provider')
      const msgRow = { ...m, type: normalizedType, sent_at: m.sent_at, created_at: m.sent_at }
      const entry = byParent.get(m.parent_id) || {
      ...
      entry.total++
      entry.messages.push(msgRow)
      // unread = parent-sent messages provider hasn't read
      if (m.type === 'parent' && m.read === false) entry.unread++
      // first iteration = latest message (DESC order)
      if (!entry.lastAt) {
        entry.lastAt = m.sent_at
        entry.lastBody = m.body || ''
        entry.lastType = normalizedType
        entry.lastSubject = m.subject || ''
      }
      byParent.set(m.parent_id, entry)
    }

    // Hydrate with parent names + children (für pfOpen-Subtitle)
    ...
    const threads = Array.from(byParent.values()).map(t => {
      const p = parentMap.get(t.parentId) as any
      return {
        ...t,
        parentName: p?.name || '',
        parentEmail: p?.email || '',
  ...
  router.post('/api/public/checkin/lookup', async (req, res) => {
    ...
    const providerName = (provider as any).displayName || (provider as any).companyName || (provider as any).name || ''
    res.json({ data: { ...result, providerName } })
  })
