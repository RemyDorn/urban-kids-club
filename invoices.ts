...
export function registerInvoiceRoutes(router: Router) {
  ...
  // DELETE: nur für Entwürfe (Drafts) — versendete/bezahlte Rechnungen müssen via /cancel storniert werden (§14 UStG)
  router.delete('/api/invoices/:id', async (req, res) => {
    const auth = await requireAuth(req, res)
    if (!auth) return
    const db = getServiceClient()
    const { data: inv } = await db.from('invoices').select('id, status, provider_id').eq('id', req.params.id).eq('provider_id', auth.providerId).maybeSingle()
    if (!inv) return res.error(404, 'Rechnung nicht gefunden')
    if (inv.status !== 'draft') return res.error(400, 'Nur Entwürfe können gelöscht werden. Versendete Rechnungen müssen storniert werden (§14 UStG).')
    const { error } = await db.from('invoices').delete().eq('id', req.params.id).eq('provider_id', auth.providerId).eq('status', 'draft')
    if (error) return res.error(500, 'Löschen fehlgeschlagen')
  ...
  router.get('/api/invoices/:id/view', async (req, res) => {
    ...
    // §14 Abs.4 Nr.6 UStG: Leistungsdatum/-zeitraum. Best-effort: aus erster verknüpfter Buchung → activity.start_at.
    // Fallback: identisch zum Rechnungsdatum (rechtlich zulässig per Hinweis).
    let serviceDate = ''
    let serviceDateNote = ''
    try {
      const bookingIds: string[] = Array.isArray(invoice.booking_ids) ? invoice.booking_ids : (invoice.booking_ids ? [invoice.booking_ids] : [])
      if (bookingIds.length > 0) {
        const { data: bk } = await db.from('provider_bookings').select('activity_id').eq('id', bookingIds[0]).maybeSingle()
        if (bk?.activity_id) {
          const { data: act } = await db.from('activities').select('start_at').eq('id', bk.activity_id).maybeSingle()
          if (act?.start_at) serviceDate = fmtDate(act.start_at)
        }
      }
    } catch { /* fall through to default */ }
    if (!serviceDate) {
      serviceDate = fmtDate(invoice.issued_at)
    ...
    const replacements: Record<string, string> = {
      ...
      '{{parentStreet}}': e(parent?.street || ''),
      '{{parentZip}}': e(parent?.zip || ''),
      '{{parentCity}}': e(parent?.city || ''),
      '{{serviceDate}}': e(serviceDate),
      '{{serviceDateNote}}': e(serviceDateNote),
    ...
    ifBlock(!!parent?.street, 'parentStreet')
    ifBlock(!!(parent?.zip || parent?.city), 'parentCityLine')
    ifBlock(!!serviceDateNote, 'serviceDateNote')
