...
// ============================================================
// Auto-Time-Entry helper:
// Whenever attendance is recorded for a session, ensure the assigned
// trainer (activity.instructor_id) gets an automatic time_entry for
// that session. Idempotent: skips if entry for that trainer+activity+date
// already exists.
// ============================================================
async function ensureAutoTimeEntry(
  sb: any,
  providerId: ID,
  activityId: ID,
  date: string,
  ...
  try {
    const { data: activity } = await sb.from('activities')
      .select('id, instructor_id, schedule').eq('id', activityId).maybeSingle()
    if (!activity || !(activity as any).instructor_id) return

    const sched = (activity as any).schedule
    const slots: any[] = Array.isArray(sched) ? sched : (sched?.slots ?? [])
    if (slots.length === 0) return

    const dow = new Date(date + 'T12:00:00').getDay()
    const dowMap: Record<number, string> = { 0: 'SU', 1: 'MO', 2: 'TU', 3: 'WE', 4: 'TH', 5: 'FR', 6: 'SA' }
    const todayCode = dowMap[dow]
    const slot = slots.find((s) => (s.day || '').toUpperCase() === todayCode)
    if (!slot?.startTime) return

    // Compute end time: prefer slot.endTime, fallback duration (default 60 min)
    let endTime: string | null = slot.endTime ?? null
    if (!endTime) {
      const [sh, sm] = String(slot.startTime).split(':').map(Number)
      const dur = Number(slot.durationMinutes || slot.duration || 60)
      const total = sh * 60 + (sm || 0) + dur
      const eh = Math.floor(total / 60), em = total % 60
      endTime = String(eh).padStart(2, '0') + ':' + String(em).padStart(2, '0')
    }

    // Verify trainer is a real team_member of this provider
    const { data: member } = await sb.from('team_members')
      .select('id, hourly_rate_cents, active')
      .eq('id', (activity as any).instructor_id)
      .eq('provider_id', providerId)
      .maybeSingle()
    if (!member || (member as any).active === false) return

    // Idempotency: skip if entry for this trainer+activity+date already exists
    ...
    await sb.from('time_entries').insert({
      ...
      started_at: `${date}T${slot.startTime}:00+02:00`,
      ended_at: `${date}T${endTime}:00+02:00`,
      source: 'auto_session',
      activity_id: activityId,
      notes: 'Automatisch erfasst beim Check-In',
      status: 'logged',
      hourly_rate_cents_snapshot: (member as any).hourly_rate_cents ?? null,
    })
  } catch (err) {
    console.error('[AutoTimeEntry] failed:', err)
  }
}
...
export const SupabaseAttendanceService = {
  ...
  async checkIn(bookingId: ID, activityId: ID, date: string, checkedInBy?: ID): Promise<AttendanceRecord | { error: string }> {
    ...
    if (existing) {
      ...
      // Fire-and-forget auto-time-entry
      if (booking?.provider_id) ensureAutoTimeEntry(sb, booking.provider_id, activityId, date)
      return fromDb(data)
    }
  ...
  async qrCheckIn(providerId: ID, email: string, bookingIds?: string[]): Promise<{
    ...
    for (const item of toCheckIn) {
      ...
      distinctActivityIds.add(item.activityId)
      checkedIn.push({ activityTitle: item.activityTitle, childName: item.childName, paymentStatus: item.paymentStatus, amountDue: item.amountDue })
    }

    // Fire-and-forget: ensure trainer time-entries exist for each distinct activity today
    for (const aid of distinctActivityIds) {
      ensureAutoTimeEntry(sb, providerId, aid, today)
    }
