...
import { escHtml, htmlPage, successPageWithRedirect, getClientIp } from './helpers'
...
export function registerWaitlistRoutes(router: Router) {
  ...
  router.get('/api/waitlist/:id/confirm', async (_req, res) => {
    ...
    try {
      ...
      await CheckoutService.createBooking({
        ...
        amount: 0,
        currency: 'EUR',
        clientIp: getClientIp(_req),
        userAgent: _req.raw?.headers?.['user-agent'] as string | undefined,
      })
    } catch (bookErr: any) {
  ...
  router.get('/api/waitlist/:id/decline-offer', async (_req, res) => {
    ...
    const { data: nextEntry } = await db.from('waitlist_entries')
      .select('id, parent_id, activity_id').eq('activity_id', entry.activity_id).eq('status', 'waiting')
    ...
    if (nextEntry) {
      try {
        ...
        await db.from('waitlist_entries').update({
          status: 'offered', notified_at: now.toISOString(),
          expires_at: expiresAt.toISOString(), confirm_token: confirmToken,
        }).eq('id', nextEntry.id)

        const { data: nextParent } = await db.from('parents').select('name, email').eq('id', nextEntry.parent_id).single()
        const { data: nextActivity } = await db.from('activities').select('title, provider_id').eq('id', nextEntry.activity_id).single()
        const { data: nextProvider } = await db.from('providers').select('company_name').eq('id', nextActivity?.provider_id).single()
        const { data: wlChild } = await db.from('waitlist_entries').select('child_info').eq('id', nextEntry.id).single()
        ...
        if (nextParent?.email) {
          ...
          await EmailService.sendWaitlistOffer(nextParent.email, {
            ...
            childName: (wlChild?.child_info as any)?.firstName || 'Ihr Kind',
          ...
          console.log('[Waitlist] Auto-offered to ' + nextParent.email + ' after decline')
        }
      } catch (autoErr) {
        console.error('[Waitlist] Auto-offer after decline failed:', autoErr)
      }
    }
  ...
  router.post('/api/waitlist/:id/offer', async (req, res) => {
    ...
    try {
      ...
      const parent = (entry as any).parents
      const activity = (entry as any).activities
      const { data: provider } = await db.from('providers').select('display_name, company_name, slug').eq('id', auth.providerId).single()
      ...
      await EmailService.sendWaitlistOffer(parent.email, {
        ...
        confirmLink,
        declineLink,
        providerId: provider?.id || activity?.provider_id,
      })
