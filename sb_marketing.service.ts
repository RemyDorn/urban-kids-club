...
export const SupabaseMarketingService = {
  ...
  async executeTemplate(
    ...
    const sbMktBrand = await getProviderBranding(providerId)
  ...
  async processTrialFollowups(providerId: ID): Promise<{
    ...
    const { EmailService, getProviderBranding } = await import('../../lib/email')
    const sbTrialBrand = await getProviderBranding(providerId)
      ...
      if (trialFlow && !followUp.feedbackSentAt && hoursSinceCompletion >= 24) {
        try {
          ...
          await EmailService.sendTrialFollowUp(parent.email, {
            ...
            providerId: trial.provider_id || (provider as any)?.id,
          })
