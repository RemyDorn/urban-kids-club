...
export const EmailService = {
  ...
  async sendCreditNotification(to: string, data: {
    parentName: string; childName: string; sessionDate: string;
    validUntil: string; providerName: string;
  }) {
    const subject = `Guthaben erhalten – ${data.childName} bei ${data.providerName}`
    const html = `
      <div style="font-family:system-ui,sans-serif;max-width:520px;margin:0 auto;color:#1f1d18">
        <h2 style="color:#d96c45;margin-bottom:8px">Guthaben erhalten</h2>
        <p>Hallo ${data.parentName},</p>
        <p>fuer den abgesagten Termin am <strong>${data.sessionDate}</strong> fuer <strong>${data.childName}</strong>
        bei <strong>${data.providerName}</strong> wurde dir ein Guthaben gutgeschrieben.</p>
        <div style="background:#fbf5ea;border-radius:12px;padding:16px;margin:16px 0;border:1px solid #e5e7eb">
          <p style="margin:0;font-size:14px"><strong>1 Credit verfuegbar</strong></p>
          <p style="margin:4px 0 0;font-size:13px;color:#67625a">Gueltig bis: ${data.validUntil}</p>
          <p style="margin:4px 0 0;font-size:13px;color:#67625a">Einloesbar als Add-Up-Termin in jedem passenden Kurs.</p>
        </div>
        <p style="font-size:13px;color:#67625a">Logge dich in deinen Bereich ein, um den Credit einzuloesen.</p>
      </div>`
