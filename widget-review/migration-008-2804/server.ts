...
// Provider-Webseite postet fbp/fbc/gclid + UTMs via postMessage, wir speichern und schicken sie mit der Buchung mit.
// Field-Format matcht den Listener-Snippet aus dem Embed-Tab im Dashboard.
  ...
  function renderStep(){
    ...
    if(step===agbStep){
      m.querySelector('#btnSubmit').onclick=async function(){
        ...
        this.disabled=true
        try{
          const r=await fetch('/api/checkout/create-session',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({slug:slug,activityId:course.id,blockId:course.blockId||null,bookedDate:window._checkoutDate||null,child:window._checkoutChild,parent:window._checkoutParent,paymentMethod:window._checkoutPayMethod,tracking:window._ukcTrack||{}})})
