...
export function registerPublicRoutes(router: Router) {
  ...
  router.post('/api/widget/waitlist', async (req, res) => {
    ...
    if (isNewParent && existingParent.id) {
      try {
        ...
        await MarketingFlowEngine.evaluateTrigger({
          ...
          eventId: existingParent.id,
          providerId: provider.id,
          recipientType: 'parent',
          recipientId: existingParent.id,
          recipientEmail: parentData.email,
          recipientName: parentData.firstName,
          templateVars: {
            parentName: parentData.firstName,
            providerName: (provider as any).displayName || (provider as any).companyName || 'Anbieter',
          },
        })
      } catch (mfErr) {
        console.error('[MarketingFlow] customer_signup (waitlist signup) failed:', mfErr)
      }
    }
  ...
  router.post('/api/checkout/create-session', async (req, res) => {
    ...
    // Add-Up-Slots (makeup_capacity) sind Provider-Buffer, NICHT öffentliche Buchungsfläche.
    // Widget-Buchungen blocken bei reiner capacity → Warteliste-Pfad. Siehe project_ukc_addup_concept.md.
    ...
    if (currentCount >= effectiveCapacity) {
      ...
      if (existingParent) {
        ...
        isNewParent2 = !!newParent?.id
      }
      // Marketing-Flow Trigger: customer_signup
      if (isNewParent2 && parentId) {
        try {
          const { MarketingFlowEngine } = await import('../../services/marketing-flow.service')
          await MarketingFlowEngine.evaluateTrigger({
            eventType: 'customer_signup',
          ...
          console.error('[MarketingFlow] customer_signup (auto-waitlist) failed:', mfErr)
        }
      }
      if (parentId) {
    ...
    // Match-Quality-Boost: Tracking-Context aus dem Request übernehmen.
    // clientIp + userAgent sind Server-Side immer verfügbar.
    // fbp/fbc/gclid/utm sind optional aus req.body.tracking (vom iFrame, Phase 2).
    const trackingCtx = {
      clientIp: ip,
      userAgent: req.raw?.headers?.['user-agent'] as string | undefined,
      fbp: (req.body as any)?.tracking?.fbp,
      fbc: (req.body as any)?.tracking?.fbc,
      gclid: (req.body as any)?.tracking?.gclid,
      utmSource: (req.body as any)?.tracking?.utm_source,
      utmMedium: (req.body as any)?.tracking?.utm_medium,
      utmCampaign: (req.body as any)?.tracking?.utm_campaign,
      // Cross-Domain external_id aus Socialy-Bridge (siehe project_socialy_ukc_bridge.md).
      // Override für sharedUserCW.externalId, sodass Meta Lead-on-Socialy ↔ Purchase-on-UKC matcht.
      externalIdOverride: (req.body as any)?.tracking?.external_id || undefined,
    }
  ...
  // Stripe Webhook — register both legacy URL (/api/webhooks/stripe) and the
    ...
    if (event?.type === 'checkout.session.completed') {
      ...
      if (meta.activity_id) {
        ...
        const { data: whBlock } = await db.from('course_blocks')
          ...
          .order('start_date', { ascending: true }).limit(1).maybeSingle()
        // Add-Up-Slots zählen NICHT als reguläre Kapazität (siehe project_ukc_addup_concept.md)
        const maxCap = whBlock?.capacity || act?.capacity || 12
      ...
      // Phase 2A.3: Billing-Address aus Stripe customer_details an CAPI durchreichen für MQ 9.5+
      const stripeAddr = (session.customer_details && session.customer_details.address) || {}
      try {
        const { CheckoutService } = await import('../../services/supabase/checkout.service')
        const booking = await CheckoutService.createBooking({
          ...
          billingCity: stripeAddr.city || undefined,
          billingZip: stripeAddr.postal_code || undefined,
          billingState: stripeAddr.state || undefined,
          billingCountry: stripeAddr.country || undefined,
  ...
  router.get('/api/widget/providers/:slug/course-blocks', async (req, res) => {
    ...
    // Count enrollments from BOTH block_enrollments AND provider_bookings
