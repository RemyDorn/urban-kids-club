...
export interface ConversionEventInput {
  ...
  user?: {
    ...
    // Adress-Felder fürs Match-Quality-Boost (Phase 2A.3). Kommen typisch aus Stripe customer_details.address
    // beim Webhook oder aus dem Eltern-Portal-Signup-Flow. Werden vor Transport sha256-hashed.
    city?: string                   // ct
    zip?: string                    // zp
    state?: string                  // st (2-letter US/DE state code falls vorhanden, sonst Bundesland)
    country?: string                // country (2-letter ISO code lowercase, z.B. 'de')
  }
  // Tracking context
  clientIp?: string
  userAgent?: string
...
// Adress-Normalisierung gemäß Meta CAPI Spec (alle lowercase, alphanumerisch, ohne Leerzeichen/Sonderzeichen)
function normalizeAddrField(s: string): string {
  return String(s || '').trim().toLowerCase().replace(/[^a-z0-9]/g, '')
}

// Country: 2-letter ISO lowercase (z.B. 'de', 'us'). Falls länger gegeben, ersten 2 Zeichen nehmen.
function normalizeCountry(s: string): string {
  const c = String(s || '').trim().toLowerCase()
  if (c.length === 2) return c
  // Mappings für häufige Fälle
  const map: Record<string, string> = { deutschland: 'de', germany: 'de', österreich: 'at', austria: 'at', schweiz: 'ch', switzerland: 'ch' }
...
export function buildPayload(input: ConversionEventInput, providerSlug: string): Record<string, any> {
  ...
  // Adress-Hashes (Match-Quality-Boost Phase 2A.3) — sha256 nach Meta-Normalisierung
  if (u.city) userData.ct_sha256 = sha256Hex(normalizeAddrField(u.city))
  if (u.zip) userData.zp_sha256 = sha256Hex(normalizeAddrField(u.zip))
  if (u.state) userData.st_sha256 = sha256Hex(normalizeAddrField(u.state))
  if (u.country) userData.country_sha256 = sha256Hex(normalizeCountry(u.country))
