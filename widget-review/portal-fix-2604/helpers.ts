...
export async function checkRoomAvailability(providerId: string, schedule: any, excludeActivityId?: string): Promise<string | null> {
  ...
  // Räume-Anzahl: bevorzugt aus rooms-Tabelle (Räume-CRUD UI im Dashboard),
  // Fallback auf providers.room_count für Provider ohne Räume-Setup, sonst 1.
  let roomCount = 0
  try {
    const { count: activeRoomCount } = await sb.from('rooms')
      .select('*', { count: 'exact', head: true })
      .eq('provider_id', providerId)
      .neq('status', 'archived')
    roomCount = activeRoomCount ?? 0
  } catch (e) {
    // ignore — fallback below
  }
  if (!roomCount) {
    const { data: prov } = await sb.from('providers').select('room_count').eq('id', providerId).maybeSingle()
    roomCount = (prov as any)?.room_count ?? 1
  }
