...
export const SupabaseTeamService = {
  ...
  async invite(id: ID, providerId: ID): Promise<{ token: string } | { error: string }> {
    ...
    try {
      ...
      const { data: provider } = await sb.from('providers').select('company_name, display_name, logo_url, email, website_url').eq('id', providerId).maybeSingle()
      const providerDisplay = (provider as any)?.display_name || provider?.company_name || 'Provider'
      const teamBrand = {
      ...
      await EmailService.send({
        to: member.email,
        subject: `Einladung: ${providerDisplay} – Dashboard-Zugang`,
        html: `
          <div style="font-family:'Inter',sans-serif;max-width:600px;margin:0 auto;color:#3C2225;">
            ...
            <div style="padding:32px;background:#FFF9F5;border-radius:0 0 16px 16px;">
              <p>Hey ${member.name},</p>
              <p><strong>${providerDisplay}</strong> hat dich als <strong>${member.role}</strong> zum Team hinzugefügt.</p>
